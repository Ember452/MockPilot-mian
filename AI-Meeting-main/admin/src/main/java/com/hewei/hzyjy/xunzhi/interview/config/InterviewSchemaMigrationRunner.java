...
/**
 * review_item 表结构自动迁移：存量库（docker 数据卷非空时 initdb 脚本不再执行）
 * 缺表时启动补建，失败仅告警不阻断启动。
 */
@Slf4j
@Component
@RequiredArgsConstructor
public class InterviewSchemaMigrationRunner implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        try {
            jdbcTemplate.execute(CREATE_REVIEW_ITEM_SQL);
        } catch (Exception e) {
            log.warn("Schema migration for review_item failed (non-blocking): {}", e.getMessage());
        }
    }
}
